...
export function BalanceCollection({ clubName, logoUrl, rows, groups, period, fromMonth, toMonth, fromLabel, toLabel, canRemind, currentMonth, paymentReady }: {
  ...
  const reminder = canRemind ? { fromMonth, toMonth, enabled: period === "range" && toMonth === currentMonth && !!paymentReady, reason: !paymentReady ? "Cần cấu hình QR và thông tin tài khoản" : period !== "range" || toMonth !== currentMonth ? "Chỉ nhắc nợ khi kỳ kết thúc ở tháng hiện tại" : "Nhắc đóng quỹ" } : undefined;
