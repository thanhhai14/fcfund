...
export type BalanceRow = { id: string; name: string; code: string; avatarVersion: number | null; charged: number; paid: number; balance: number; openingBalance: OpeningBalance | null; cells: Array<BalanceCell & { typeId: string }> };
...
function BalanceReportTable({ rows, groups, isColumnVisible, snapshot = false }: {
  ...
  return <table className={`${snapshot ? "balance-snapshot-table" : "monthly-table balance-period-table report-sticky-table"} ${isColumnVisible("rank") ? "has-rank-column" : ""}`}>
    <thead>
      <tr>{isColumnVisible("rank") && <th rowSpan={cellColumnCount ? 3 : 1} className={snapshot ? "snapshot-rank" : "report-rank-column"}>Hạng</th>}<th rowSpan={cellColumnCount ? 3 : 1} className={snapshot ? "snapshot-member" : "report-member-column"}>Thành viên</th>{showOutside && <th rowSpan={cellColumnCount ? 3 : 1}>Số dư trước kỳ</th>}{cellColumnCount > 0 && <th colSpan={cellColumnCount} className="balance-charge-super-header">Các khoản phát sinh</th>}{isColumnVisible("charged") && <th rowSpan={cellColumnCount ? 3 : 1}>Phát sinh</th>}{isColumnVisible("paid") && <th rowSpan={cellColumnCount ? 3 : 1}>Đã đóng</th>}{isColumnVisible("balance") && <th rowSpan={cellColumnCount ? 3 : 1}>Số dư cuối kỳ</th>}</tr>
