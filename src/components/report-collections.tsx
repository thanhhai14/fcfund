...
export function BalanceCollection({ clubName, logoUrl, rows, groups, period, fromMonth, toMonth, fromLabel, toLabel }: {
  ...
  return <article className="panel table-panel balance-collection">
    <div className="monthly-report-heading balance-report-heading"><div><span className="eyebrow">Công nợ theo khoảng tháng</span><h2>{periodLabel}</h2><p>Các khoản phát sinh và tiền đã đóng trong cùng khoảng thời gian</p></div><div className="monthly-report-actions"><form action="/reports" method="get" className="balance-period-controls"><input type="hidden" name="tab" value="balances" /><select name="balancePeriod" value={selectedPeriod} onChange={(event) => setSelectedPeriod(event.target.value as "range" | "all")} aria-label="Phạm vi công nợ"><option value="range">Theo khoảng tháng</option><option value="all">Toàn bộ</option></select>{selectedPeriod === "range" && <><label><span>Từ</span><input type="month" name="balanceFromMonth" value={selectedFromMonth} onChange={(event) => setSelectedFromMonth(event.target.value)} aria-label="Từ tháng" required /></label><label><span>Đến</span><input type="month" name="balanceToMonth" value={selectedToMonth} onChange={(event) => setSelectedToMonth(event.target.value)} aria-label="Đến tháng" required /></label></>}<button className="button secondary small">Xem</button></form><ReportImageExporter title={`Báo cáo công nợ · ${periodLabel}`} subtitle={`${visible.length} thành viên · Theo bộ lọc và thứ tự đang hiển thị`} clubName={clubName} logoUrl={logoUrl} filename={`bao-cao-cong-no-${period === "all" ? "toan-bo" : `${fromMonth}_${toMonth}`}.png`} width={exportWidth}><BalanceReportSnapshot rows={visible} groups={groups} isColumnVisible={columns.isVisible} /></ReportImageExporter></div></div>
