...
const SEED_COLUMNS: CollectionColumn[] = [{ id: "member", label: "Thành viên", required: true }, { id: "seed", label: "Seed trận này", required: true }, { id: "matches", label: "Số trận" }, { id: "wins", label: "Hạng nhất" }, { id: "formScore", label: "Phong độ" }];

export function SeedEvaluationTable({ rows }: { rows: SeedRow[] }) {
  const [query, setQuery] = useState("");
  const [filter, setFilter] = useState("ALL");
  const [sort, setSort] = useState("NAME");
  const [seeds, setSeeds] = useState<Record<string, string>>(() => Object.fromEntries(rows.map((row) => [row.id, row.seedTier ?? row.suggestedSeedTier ?? ""])));
  ...
  return <>
    ...
    <div className="seed-table-wrap"><table className="seed-table"><thead><tr><th>Thành viên</th><th>Seed trận này</th>{columns.isVisible("matches") && <th>Số trận</th>}{columns.isVisible("wins") && <th>Hạng nhất</th>}{columns.isVisible("formScore") && <th>Phong độ</th>}</tr></thead><tbody>{ordered.map((row) => {
