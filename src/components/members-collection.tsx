      ...
      {view === "list" ? (
        <div className="member-list-view">
          ...
          {visible.map((member, index) => <Link href={`/members/${member.id}`} className="member-list-row" style={{ gridTemplateColumns, minWidth: gridMinWidth }} key={member.id}>
            ...
            {columns.isVisible("foot") && <span className="preferred-foot-value">{member.preferredFoot ? <><PreferredFootIcon value={member.preferredFoot} />{preferredFootLabel(member.preferredFoot)}</> : "—"}</span>}
