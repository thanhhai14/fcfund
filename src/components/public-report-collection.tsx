...
export type PublicReportRow = { id: string; name: string; avatarVersion: number | null; charged: number; paid: number; balance: number; openingBalance: OpeningBalance | null; cells: PublicReportCell[] };

function visiblePublicCharged(row: PublicReportRow, isColumnVisible: (id: string) => boolean) {
  return row.cells.reduce((sum, cell) => sum + (isColumnVisible(balanceCellColumnId(cell.month, cell.typeId, cell.sourceMonth)) ? cell.total : 0), 0);
}

function visiblePublicBalance(row: PublicReportRow, isColumnVisible: (id: string) => boolean) {
  return (row.openingBalance?.amount ?? 0) + row.paid - visiblePublicCharged(row, isColumnVisible);
}
...
function PublicReportTable({ rows, groups, isColumnVisible, snapshot = false }: { rows: PublicReportRow[]; groups: PublicReportGroup[]; isColumnVisible: (id: string) => boolean; snapshot?: boolean }) {
  ...
  const totalCharged = rows.reduce((sum, row) => sum + visiblePublicCharged(row, isColumnVisible), 0);
  ...
  return <table className={`public-report-table ${snapshot ? "public-report-table-snapshot" : ""}`}>
    <thead>
      <tr>{isColumnVisible("rank") && <th rowSpan={cellCount ? 3 : 1} className="public-report-rank-column">Hạng</th>}<th rowSpan={cellCount ? 3 : 1} className="public-report-member-column">Thành viên</th>{showOutside && <th rowSpan={cellCount ? 3 : 1}>Số dư trước kỳ</th>}{cellCount > 0 && <th colSpan={cellCount} className="public-report-charge-header">Các khoản phát sinh</th>}{isColumnVisible("charged") && <th rowSpan={cellCount ? 3 : 1}>Phát sinh</th>}{isColumnVisible("paid") && <th rowSpan={cellCount ? 3 : 1}>Đã đóng</th>}{isColumnVisible("balance") && <th rowSpan={cellCount ? 3 : 1}>Số dư cuối kỳ</th>}</tr>
...
function PublicReportSnapshot({ rows, groups, isColumnVisible }: { rows: PublicReportRow[]; groups: PublicReportGroup[]; isColumnVisible: (id: string) => boolean }) {
  const charged = rows.reduce((sum, row) => sum + visiblePublicCharged(row, isColumnVisible), 0);
...
function PublicReportCard({ row, groups, rank, showOutside }: { row: PublicReportRow; groups: PublicReportGroup[]; rank: number; showOutside: boolean }) {
  return <article className="public-report-member">
    ...
    {showOutside && row.openingBalance && <div className={`outside-period-card-note${row.openingBalance.amount > 0 ? " opening-credit" : ""}`}><span>Số dư trước kỳ</span><OpeningBalanceBadge balance={row.openingBalance} /></div>}
    ...
    <footer><span><small>Phát sinh</small><b>{formatMoney(row.charged)}</b></span><span><small>Đã đóng</small><b>{formatMoney(row.paid)}</b></span><span><small>Số dư cuối kỳ</small><b className={row.balance < 0 ? "money-out" : "money-in"}>{row.balance > 0 ? "+" : ""}{formatMoney(row.balance)}</b></span></footer>
  </article>;
}
...
export function PublicReportCollection({ clubName, appName, logoUrl, rows, groups, period, fromMonth, toMonth, fromLabel, toLabel }: {
  ...
  const columnDefinitions = useMemo<CollectionColumn[]>(() => [
    ...
    { id: "charged", label: "Phát sinh" },
    { id: "paid", label: "Đã đóng" },
    { id: "balance", label: "Số dư cuối kỳ" },
  ], [groups, rows]);
  ...
  const visible = useMemo(() => {
    ...
    const isVisible = (id: string) => !columns.hidden.includes(id);
    const result = rows.filter((row) => !normalized || normalizeSearch(row.name).includes(normalized)).map((row) => ({
      ...row,
      charged: visiblePublicCharged(row, isVisible),
      balance: visiblePublicBalance(row, isVisible),
    }));
  ...
  return <main className="public-report-page">
    <header className="public-report-hero">
      <div className="public-report-brand"><PublicClubLogo name={clubName} src={logoUrl} /><div><small>BÁO CÁO CÔNG KHAI</small><h1>{clubName}</h1><p>{appName} · Báo cáo theo kỳ</p></div></div>
      ...
      <div className="public-report-hero-actions"><ReportImageExporter iconOnly title={captureTitle} subtitle={`${visible.length} thành viên · Theo bộ lọc và thứ tự đang hiển thị`} clubName={clubName} logoUrl={logoUrl} filename={`bao-cao-theo-ky-${period === "all" ? "toan-bo" : `${fromMonth}_${toMonth}`}.png`} width={exportWidth}><PublicReportSnapshot rows={visible} groups={groups} isColumnVisible={columns.isVisible} /></ReportImageExporter></div>
    </header>
    <section className="public-report-summary"><div><small>THÀNH VIÊN</small><strong>{visible.length}</strong></div><div><small>TỔNG PHÁT SINH</small><strong>{formatMoney(visible.reduce((sum, row) => sum + row.charged, 0))}</strong></div><div><small>ĐÃ ĐÓNG</small><strong>{formatMoney(visible.reduce((sum, row) => sum + row.paid, 0))}</strong></div><div><small>SỐ DƯ CUỐI KỲ</small><strong className={visible.reduce((sum, row) => sum + row.balance, 0) < 0 ? "money-out" : "money-in"}>{formatMoney(visible.reduce((sum, row) => sum + row.balance, 0))}</strong></div></section>
    <p className="public-report-note"><Icon name="info" /> Số dư cuối kỳ = số dư trước kỳ + tiền đã đóng trong kỳ − phát sinh trong kỳ. Khoản chuyển kỳ được tổng kết vào tháng kế tiếp.</p>
    ...
    {view === "list" ? <div className="public-report-table-wrap"><PublicReportTable rows={visible} groups={groups} isColumnVisible={columns.isVisible} /></div> : <section className="public-report-members" aria-label="Báo cáo theo kỳ từng thành viên">{visible.map((row, index) => <PublicReportCard key={row.id} row={row} groups={displayGroups} rank={index + 1} showOutside={showOutside} />)}{!visible.length && <p className="public-report-empty">Không tìm thấy thành viên phù hợp.</p>}</section>}
